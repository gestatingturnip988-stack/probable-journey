...
// Start game ONLY when mobile DOM is 100% loaded
document.addEventListener('DOMContentLoaded', () => {
    try {
        const container = document.getElementById('canvas-container');
        if (!container) {
            showMobileError("Canvas container #canvas-container not found in HTML!");
            return;
        }
        ...
        function updateInventoryUI() {
            ...
            for (let i = invGrid.children.length; i < 16; i++) {
                const slot = document.createElement('div');
                slot.className = 'inv-item-slot';
        ...
        function triggerHit(rangeBonus, damageVal) {
        ...
        bindActionPointer('btn-action-1', () => { if (consumeEnergy(10)) { createSlashFX(1.8, 0x00ffcc); triggerHit(1.8, 1); } });
        bindActionPointer('btn-action-2', () => { if (consumeEnergy(25)) { createSlashFX(2.6, 0xff3300); triggerHit(2.6, 2.5); } });
        bindActionPointer('btn-action-3', () => { if (consumeEnergy(8)) { createSlashFX(1.5, 0xffaa00); triggerHit(1.5, 1); } });
